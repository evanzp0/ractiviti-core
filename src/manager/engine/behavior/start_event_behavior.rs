...
impl StartEventBehavior {
    ...
    pub async fn execute<'a>(&mut self, operator_ctx: &mut OperatorContext, tran: &mut Transaction<'a, Postgres>)
            -> Result<()> {
        // create current execution
        let start_time= get_now();
        let start_user = operator_ctx.user_id.clone();
        let current_execution = self.base.create_current_execution(
            &self.base.element.get_element_id(), start_time, start_user, tran).await?;
        ...
        debug!("StartEvent (process: {:?}, element: {}) is executed",
