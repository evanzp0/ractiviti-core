...
use crate::manager::engine::{BaseOperator, BpmnElement, ContinueProcessOperator, OperateRst, Operator, OperatorContext};
...
impl TakeOutgoingFlowsOperator {
    pub fn new(element: BpmnElement, proc_inst: Arc<ApfRuExecution>,
    ...
    pub async fn execute<'a> (&self, operator_ctx: &OperatorContext, tran: &mut Transaction<'a, Postgres>)
            -> Result<OperateRst> {
        ...
        match &self.base.element {
            BpmnElement::Edge(edge) => {
                ...
                debug!("Sequence Flow (process: {:?}, element: {}, source: {}, target: {})",
                ...
                if let BpmnElement::Node(node) = target_element {
                    ...
                    self.base.mark_begin_exection(&node.get_id(),
