...
impl CreateTaskCmd {
    ...
    pub async fn execute<'a> (&self, operator_ctx: &mut OperatorContext, tran: &mut Transaction<'a, Postgres>)
        ...
        let task = task_dao.create(&new_ru_task, tran).await?;

        // create task history
        let hi_task_dao = ApfHiTaskinstDao::new();
        hi_task_dao.create_from_task(&task, tran).await?;
            ...
            match node.get_node_type() {
                NodeType::UserTask | NodeType::ServiceTask => {
                    for group in node.candidate_groups().iter() {
                        ...
                        let ru_ident = ru_ident_dao.create(&new_ru_ident, tran).await?;
                        hi_ident_dao.create_from_ident_link(&ru_ident, tran).await?;
                    }
