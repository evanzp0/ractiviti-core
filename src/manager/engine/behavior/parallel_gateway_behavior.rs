...
use crate::manager::engine::{BaseOperator, BpmnElement, OperateRst, Operator, OperatorContext, TakeOutgoingFlowsOperator};
...
impl ParallelGatewayBehavior {
    ...
    pub async fn execute<'a>(&self, operator_ctx: &mut OperatorContext, tran: &mut Transaction<'a, Postgres>)
        ...
        if let BpmnElement::Node(node) = &self.base.element {
            ...
            let inactive_exec_count = exec_dao.count_inactive_by_element(&proc_inst_id, &element_id, tran).await?;
            ...
            if inactive_exec_count + 1 < in_flows_count {
                ...
                exec_dao.deactive_execution(current_execution_id, tran).await?;
            } else {
                exec_dao.del_inactive_by_element(&proc_inst_id, &element_id, tran).await?;

                // #[cfg(debug_assertions)]
                debug!("ParallelGateway (process: {:?}, element: {})",
    ...
    async fn leave<'a>(&self, operator_ctx: &mut OperatorContext, tran: &mut Transaction<'a, Postgres>)
                -> Result<OperateRst> {
        ...
        if let BpmnElement::Node(node) = element {
            ...
            if out_flows.is_empty() {
                Err(AppError::new(ErrorCode::NotFound,
                          Some(&format!("not found valid outflows for parallel gateway (proc_inst: {:?}, element: {})",
            ...
            let first_flow = out_flows.get(0)
                .ok_or(AppError::new(ErrorCode::NotFound,
                                     Some("unexpected error"), "", None))?;

            self.base.mark_begin_exection(&first_flow.get_id(),
                                          operator_ctx.user_id.clone(),
                                          start_time.clone(), tran).await?;
            ...
            for flow in &out_flows[1..] {
                let current_exec = self.base.create_current_execution(
                    &flow.get_id(),
                    start_time.clone(),
                    operator_ctx.user_id.clone(), tran).await?;
