...
use crate::manager::engine::{CompleteTaskCmd, Operator, OperatorContext, OperatorExecutor, ProcessEngine, TypeWrapper};
use crate::model::{ApfRuVariable, ApfRuVariableDto, VarType};
...
impl TaskService {
    ...
    pub async fn _complete<'a>(&self, task_id: &str, operator_ctx: &mut OperatorContext, tran: &mut Transaction<'a, Postgres>) -> Result<()> {
        ...
        let re_def = procdef_dao.get_by_id(&current_task.proc_def_id, tran).await?;
        ...
        for (key, value) in operator_ctx.variables.read().unwrap().iter() {
            let dto = ApfRuVariableDto {
                var_type: VarType::from(value.clone()),
                name: key.to_owned(),
                value: value.as_str(),
            ...
            hi_var_dao.create_or_update_by_variable(&variable, update_time, tran).await?;
        }

        let var_insts = var_dao.find_all_by_proc_inst(&current_task.proc_inst_id, tran).await?;
        ...
        let execution_dao = ApfRuExecutionDao::new();
...
mod tests {
    ...
    async fn test_complete() {
        ...
        variables.insert("approval_pass".to_owned(), TypeWrapper::bool(false));
