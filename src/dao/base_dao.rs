...
impl BaseDao {
    pub fn bind_params<'a, T>(mut query: QueryAsType<'a, T>,
                              params: &Vec<Box<dyn Any>>)
            -> Result<QueryAsType<'a, T>> {
        for param in params {
            ...
            if TypeId::of::<Option<String>>() == param_type_id {
                ...
                query = query.bind(p);
            } else if TypeId::of::<Option<Uuid>>() == param_type_id {
                let p = *param.downcast_ref::<Option<Uuid>>().ok_or(AppError::unexpected_error(concat!(file!(), ":", line!())))?;
                query = query.bind(p);
            } else {
    ...
    pub fn bind_params_scalar<'a, T>(mut query: QueryScalarAsType<'a, T>,
                              params: &Vec<Box<dyn Any>>)
                              -> Result<QueryScalarAsType<'a, T>> {
        for param in params {
            let param_type_id = (**param).type_id();
    ...
    fn bind_prarms_raw<'a, T>(mut query: QueryAsType<'a, T>, param: &Box<dyn Any>) -> Result<QueryAsType<'a, T>> {
        ...
        if TypeId::of::<String>() == param_type_id {
            ...
            query = query.bind(p);
        } else if TypeId::of::<Uuid>() == param_type_id {
            let p = *param.downcast_ref::<Uuid>().ok_or(AppError::unexpected_error(concat!(file!(), ":", line!())))?;
            query = query.bind(p);
        } else {
    ...
    pub async fn find_by_crieria<'a, T>(sql: &str, params: &Vec<Box<dyn Any>>,
                                        tran: &mut Transaction<'a, Postgres>)
                                        -> Result<Vec<T>>
            where T: for<'r> FromRow<'r, PgRow> + Send + Unpin {
        let mut query = sqlx::query_as::<_, T>(sql);
        query = BaseDao::bind_params(query, params)?;
        let rst = query.fetch_all(&mut *tran).await?;

        Ok(rst)
    }

    pub async fn fetch_one_by_crieria<'a, T>(sql: &str, params: &Vec<Box<dyn Any>>,
                                             tran: &mut Transaction<'a, Postgres>)
                                             -> Result<T>
            where T: for<'r> FromRow<'r, PgRow> + Send + Unpin {
    ...
    pub async fn fetch_scalar_by_crieria(sql: &str, params: &Vec<Box<dyn Any>>,
                                             tran: &mut Transaction<'_, Postgres>)
                                             -> Result<i64> {
        let mut query = sqlx::query_scalar::<_, i64>(sql);
        query = BaseDao::bind_params_scalar(query, params)?;
        let rst = query.fetch_one(&mut *tran).await?;
