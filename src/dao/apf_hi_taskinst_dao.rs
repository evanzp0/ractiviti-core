...
impl ApfHiTaskinstDao {
    ...
    pub async fn create_from_task(&self, task: &ApfRuTask, tran: &mut Transaction<'_, Postgres>)
            -> Result<ApfHiTaskinst> {
    ...
    pub async fn create(&self, obj: &NewApfHiTaskinst, tran: &mut Transaction<'_, Postgres>)
                            -> Result<ApfHiTaskinst> {
        let sql = "insert into apf_hi_taskinst \
    ...
    pub async fn mark_end(&self, task_id: &Uuid, end_user_id: Option<String>, tran: &mut Transaction<'_, Postgres>)
                              -> Result<()> {
        ...
        let sql = "update apf_hi_taskinst \
                         set end_time = $1, \
                             duration = $2,\
                             end_user_id = $3, \
                             rev = rev + 1 \
        ...
        if rst.rows_affected() != 1 {
            Err(AppError::new(ErrorCode::InternalError,
                              Some(&format!("apf_hi_taskinst({}) is not updated correctly, affects({}) != 1",
                                            task_id, rst.rows_affected())),
                              concat!(file!(), ":", line!()),
                              None))?
        }
...
pub mod tests {
    ...
    pub async fn create_test_hi_task(task: &ApfRuTask, mut tran: &mut Transaction<'_, Postgres>)
                                         -> ApfHiTaskinst {
        let hi_task_dao = ApfHiTaskinstDao::new();
