...
drop table if exists ApfRuVariable;
...
create table apf_re_procdef (
    ...
    deployment_id uuid not null references apf_re_deployment(id),
...
create table apf_ru_execution (
    id uuid default uuid_generate_v4() not null primary key,
    rev int default null,
    proc_inst_id uuid null references apf_ru_execution(id),
    business_key varchar(255) null,
    parent_id uuid null references apf_ru_execution(id),
    proc_def_id uuid null references apf_re_procdef(id),
    root_proc_inst_id uuid null,
...
create table apf_ru_task (
    id uuid default uuid_generate_v4() not null primary key,
    rev int default null,
    execution_id uuid not null references apf_ru_execution(id),
    proc_inst_id uuid not null references apf_ru_execution(id),
    proc_def_id uuid not null references apf_re_procdef(id),
    element_id varchar(255) not null,
    element_name varchar(255) null,
    element_type varchar(255) null,
    business_key varchar(255) null,
...
create table apf_ru_variable (
    ...
    proc_inst_id uuid not null references apf_ru_execution(id),
    task_id uuid null , -- 任务结束后，任务会被删除，但是变量需要留着给后续流程使用
...
create table apf_ru_identitylink (
    ...
    task_id uuid null references apf_ru_task(id),
    proc_inst_id uuid null references apf_ru_execution(id),
    proc_def_id uuid null references apf_re_procdef(id)
);
